...
input <- args[1]
sampleSheet <- paste0("/haplox/users/yangbo/dezip/",input,".csv")
csv_df <- read.csv(sampleSheet, header=FALSE, stringsAsFactors = FALSE,  fileEncoding="GBK")
out_csv <- paste0("/haplox/users/yangbo/dezip/",input,"_up_oss.csv")
rawrfqz <- NULL
if(nrow(csv_df)>=1){
...
write.table(result,file=out_csv, fileEncoding="GBK",row.names=FALSE,col.names=FALSE, na="", quote = F,sep=",")
if(nrow(csv_df) >=1){
  for(i in seq(nrow(csv_df))){
    oss_download <- paste0("/haplox/users/yangbo/dezip/download_",input,".sh")
    sink(oss_download,append=TRUE)
    if(csv_df[i,11]==0){
      cat(paste0("###\n","ossutil cp -ru oss://sz-hapres/haplox/hapyun/201911/tmp_workflow_repaq-xz-rawfq_1_",input,"/tmp/repaq-xz-rawfq_node_2/ --include ",
