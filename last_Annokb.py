...
def Annokb(path,hap,dat,gdna,num):
    if hap != gdna:
        ...
        {S1}/{S3}.snv-nobias-GB18030-baseline-genes378.csv 0.1'.format(S1=path,S3=dat))

        os.system('Rscript /haplox/users/huang/mypy/data-analysis/ctdna_exome_pipeline/getFFPE_cfDNA_snv_indel_v2.R \
        /haplox/users/huang/mypy/data-analysis/ctdna_exome_pipeline/ffpe_vs_pbl/genes378.bed \
        {S1}/{S3}.indel-nobias-GB18030-baseline.csv T \
        {S1}/{S3}.indel-nobias-GB18030-baseline-genes378.csv 0.1'.format(S1=path,S3=dat))
    else:
        os.system('Rscript /haplox/users/huang/mypy/data-analysis/ctdna_exome_pipeline/getFFPE_cfDNA_snv_indel_v2.R \
        /haplox/users/huang/mypy/data-analysis/ctdna_exome_pipeline/ffpe_vs_pbl/genes378.bed \
        {S1}/{S3}_snv-GB18030-baseline.csv T \
        {S1}/{S3}.snv-nobias-GB18030-baseline-genes378.csv 0.5'.format(S1=path,S3=dat))

        os.system('Rscript /haplox/users/huang/mypy/data-analysis/ctdna_exome_pipeline/getFFPE_cfDNA_snv_indel_v2.R \
        /haplox/users/huang/mypy/data-analysis/ctdna_exome_pipeline/ffpe_vs_pbl/genes378.bed \
        {S1}/{S3}_indel-GB18030-baseline.csv T \
        {S1}/{S3}.indel-nobias-GB18030-baseline-genes378.csv 0.5'.format(S1=path,S3=dat))


    #-----------------------------------------------------------------------------------------      
    os.system('python /thinker/net/tools/Annokb.py \
    -f {S1}/{S3}.indel-nobias-GB18030-baseline-genes378.csv  \
    -t indel -o {S1}/Annokb_mrbam_{S3}.indel-nobias-GB18030-baseline-genes378.csv'.format(S1=path,S3=dat))

    os.system('python /thinker/net/tools/Annokb.py \
    -f {S1}/{S3}.snv-nobias-GB18030-baseline-genes378.csv  \
    -t snv -o {S1}/Annokb_mrbam_{S3}.snv-nobias-GB18030-baseline-genes378.csv'.format(S1=path,S3=dat))
    ...
    cnv_list = path + '/cnv/' + hap + '_rg_cnv_geneschrX.csv'
    avg = cnv(cnv_list)
...
def cnv_updata(path,num):
    ...
    for i in os.listdir(cnv_dir):
        if i.endswith("_rg_cnv_result.csv"):
