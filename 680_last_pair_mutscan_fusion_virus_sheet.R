...
sheet_dir <- paste0(path,data_id,"_680")
...
if(nrow(tumor_df)>=0){
    for(i in seq(nrow(tumor_df))){
        if(tumor_df[i,1] == sample){
            system(paste0("Rscript /haplox/users/yangbo/680_last_mutscan_fusion_virus.R ",path,"\n",
...
#            "cp ",path,"germline/result/",normal,".Target_451.xls ",sheet_dir,"\n",
            ...
            "cp ",path,"germline/result/",normal,"_trans.germline_680.cancer.utf8vep.xls " ,sheet_dir,"\n",
            "cp ",path,"germline/result/",normal,"_trans_germline_DDR_cancer_utf8vep.xls " ,sheet_dir,"\n",
            ...
            "cp -r ",path,"SZ*-nobias-GB18030-baseline* ",sheet_dir,"\n"))
