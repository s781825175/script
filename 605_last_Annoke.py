...
def cnv_updata(path,num,sample,tmp):
            ...
            with open(cnv_dir + i) as cnv_list:
                ...
                for row in point_reader:
                    ...
                    if float(row[1]) >= 3:
                        ...
                        a += 1
                    if float(row[1]) <= 1.3 and row[0] != 'ATRX' and row[0] != 'AR':
                        if sample == 'ffpe':
...
def diffpanel(path,num,sample):
    ...
    tmp = 'cnv_single'
    cnv_updata(path,num,sample,tmp)
...
def main():
    ...
    for i in os.listdir(gdna_dir):
        if i.endswith('chem_451.txt'):
            gdna = i.split('.')[0]
    num = hap.split('_')[-1]
    sample = Annokb(path,hap,dat,gdna,num)
    ...
    if sample == 'ffpe':  #把血液栏的丰度转移到组织栏
        for i in os.listdir(path):
            if 'Annokb_mrbam' in i:
                ...
                Annokb_file.close()    
    os.system('Rscript /haplox/users/zhaoys/Script/last_pair_mutscan_fusion_virus_sheet.R {S1}/'.format(S1=path))
    tmp = 'cnv'
    cnv_updata(path,num,sample,tmp)
