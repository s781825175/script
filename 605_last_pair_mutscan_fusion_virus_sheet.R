...
tumor_df <- csv_df[grepl("cfdna|healthcfdna|pedna|ttdna|ffpedna",csv_df[,1]) & grepl("451plus|605panel|680",csv_df[,1]),]
germline_path <- paste0(path,"germline/result/")
cmd <- paste0("ls ",germline_path,"*_trans.germline_CIST58.cancer.txt")
all_tumor <- system(cmd,intern = TRUE)
normal <- gsub(germline_path,"",strsplit(all_tumor,"_trans.germline_CIST58.cancer.txt")[[1]][1])
print(normal)
...
if(nrow(tumor_df)>=0){
    for(i in seq(nrow(tumor_df))){
        if(tumor_df[i,1] == sample){
            system(paste0("Rscript /haplox/users/yangbo/605_last_mutscan_fusion_virus.R ",path,"\n",
            ...
            "cp ",path,"cnv/",sample,"_rg_cnv.genes464.csv ",sheet_dir,"\n",
#            "cp ",path,"cnv/",sample,"_rg_cnv.genes188in680.csv ",sheet_dir,"\n",
#            "cp ",path,"cnv/",sample,"_rg_cnv.genes188in680_gain.csv ",sheet_dir,"\n",
#            "cp ",path,"germline/result/",normal,"_trans.germline_680.cancer.xls " ,sheet_dir,"\n",
#            "cp ",path,"germline/result/",normal,"_trans.germline_DDR.cancer.xls " ,sheet_dir,"\n",
#            "cp ",path,"/HLA/",normal,"_sortbam-hla.csv " ,sheet_dir,"\n",
            ...
            if(grepl("男",tumor_df[i,5])){
                system(paste0("cp ",path,"germline/result/",normal,"_trans_cancer_male_utf8vep.xls ",sheet_dir))
            }
            if(grepl("女",tumor_df[i,5])){
                system(paste0("cp ",path,"germline/result/",normal,"_trans_cancer_female_utf8vep.xls ",sheet_dir))
            }
        }
    }
