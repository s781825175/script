                ...
                for k in j.split(':'):
                    if '%' in k:
                        if sample_name in j.split(':')[0]:
                            ...
                            order.append(float(k))
            if order == []:
                clean_data.append(i)
                continue
            elif float(sample[0]) < 0.2 and (len(tmp_data.split('\n')) < 5 or (int(sample[1]) > 5 and sum(order[1:][::2])/len(order[::2]) < 2 )):
                clean_data.append(i)
                continue
            else:
                filter_data.append(i)
...
def Annokb(path,hap,dat,gdna,num):
    ...
    #    grep_sample('Annokb_mrbam_{S3}.snv-nobias-GB18030-baseline-genes528in680.csv'.format(S3=dat),path,dat)
    ...
    with open(cnv_file) as cnv_list:
        ...
        for row in point_reader:
            if a==0:
                ...
                continue
            if float(row[1]) >= 3 and (float(row[1]) >= 5 or row[0] not in amplification_black):
                if sample == 'ffpe':
